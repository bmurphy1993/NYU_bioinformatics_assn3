...
t2g_1 <- biomaRt::getBM(attributes = c("ensembl_transcript_id", "transcript_version", "ensembl_gene_id", "external_gene_name"), mart = mart)
t2g_2 <- biomaRt::getBM(attributes = c("ensembl_transcript_id", "description", "transcript_biotype", "refseq_mrna", "refseq_ncrna"), mart = mart)
t2g <- merge(t2g_1, t2g_2, by="ensembl_transcript_id")
...
write.table(round(counts),paste("t2g_output",".txt",sep=""), row.names=ids, quote=F, col.names=T, sep="\t")
